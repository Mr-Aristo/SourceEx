...
- EF migration dosyaları repo içinde henüz yok.
...
Şu an logging temel host logging ve consumer logger’ları ile yapılmış. API’de ayrıca `AddHttpLogging()` var.
...
Bu yüzden sistem izlenebilirliği temel seviyede.
...
Şu an repo’da unit test veya integration test projeleri yok. Bu kadar katmanlı bir yapıda test eksikliği ciddi bir boşluk.

#### 2. Migration dosyaları yok

EF Core migration’larının commit edilmemiş olması ilk kurulum maliyetini artırır.
...
#### 6. Observability temel seviyede

Log var, ama sistem davranışını izlemek için daha fazlası gerekir.

#### 7. CQRS fiziksel ayrışmaya gitmemiş

Bu kötü değil, ama beklentiyi doğru koymak gerekir. Şu an read ve write aynı veri modelinden çalışıyor.

#### 8. Bazı isimlendirme ve yapı tutarsızlıkları var
